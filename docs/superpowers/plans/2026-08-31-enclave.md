...
**Tech Stack:** Vite 7 · React 19 · TypeScript · Tailwind 4 (`@tailwindcss/vite`)
· Zustand 5 · DuckDB-WASM 1.32.0 · ECharts 6 + echarts-for-react 3 · openai 7 ·
Vitest 3 · Playwright (dev only) · npm

> **Toolchain note, settled in Task 1.** Vite 8 is rolldown-based and Vitest
> does not support it yet: npm silently resolved Vitest down to 3.2.7 while
> leaving `@vitest/coverage-v8` at 4, and the `test` key stopped type-checking
> because Vitest augments its own nested copy of Vite. We pin **Vite 7 +
> Vitest 3** and drop the coverage package. Playwright is installed as a dev
> dependency so browser-only behaviour can be verified from the terminal via
